...
def typeOfVehicle(flag):
    return random.choice(["carro", "moto", "otro", "varios"]) if flag else "N/A"
...
def livingPlace(type):
    ...
    return random.choice(types) if type != "N/A" else random.choice(["Inquilinato", "Campamento", "Residencia Estudiantil",
...
def etnia():
    options = ["Negro", "Mulato", "Afrocolombiano", "Afroamericano", "Indigena", "Raizal", "Gitano", "Palenquero", "N/A"]
...
def typeOfDisability():
    return random.choice(["Fisicas", "Sensorial", "Psiquicas"])
...
def countryOfBirth():
    ...
    return random.choices(countries, weights=(10,50,10,10,10,10,10))

def allergies():
    comAllergies = ["N/A", "Polen", "Cosmeticos", "Polvo", "Alimentos", "Gluten", "Medicamentos", "Detergentes Ropa", "Animales", "Moho", "Fragancias"]
    random.choice(comAllergies)
